...
var Calendar = function(model, options, date) {
	...
	this.Selected.Days = new Date(this.Selected.Year,
			(this.Selected.Month + 1), 0).getDate();
	this.Selected.FirstDay = new Date(this.Selected.Year,
			(this.Selected.Month), 1).getDay();
	this.Selected.LastDay = new Date(this.Selected.Year,
			(this.Selected.Month + 1), 0).getDay();

	this.Prev = new Date(this.Selected.Year, (this.Selected.Month - 1), 1);
	if (this.Selected.Month == 0) {
		this.Prev = new Date(this.Selected.Year - 1, 11, 1);
	}
	this.Prev.Days = new Date(this.Prev.getFullYear(),
			(this.Prev.getMonth() + 1), 0).getDate();

};

function createCalendar(calendar, element, adjuster) {
	if (typeof adjuster !== 'undefined') {
		var newDate = new Date(calendar.Selected.Year, calendar.Selected.Month
				+ adjuster, 1);
	...
	function AddSidebar() {
		...
		for (var i = 0; i < months.length - 3; i++) {
			...
			if (i == 0) {
				...
				// prevent losing var adj value (for whatever reason that is
				// happening)
				(function() {
					var adj = (i - 4);
		...
		if (calendar.Options.NavLocation) {
			...
			document.getElementById(calendar.Options.NavLocation).appendChild(
					sidebar);
		} else {
	...
	function AddDays() {
		...
		for (var i = 0; i < (calendar.Selected.FirstDay); i++) {
			...
			var number = DayNumber((calendar.Prev.Days - calendar.Selected.FirstDay)
					+ (i + 1));
		...
		for (var i = 0; i < calendar.Selected.Days; i++) {
			...
			for (var n = 0; n < calendar.Model.length; n++) {
				...
				var toDate = new Date(calendar.Selected.Year,
						calendar.Selected.Month, (i + 1));
					...
					if (typeof calendar.Model[n].Link == 'function'
						...
						if (calendar.Options.EventClick) {
							...
							if (typeof calendar.Model[n].Link != 'string') {
								...
								a.addEventListener('click',
										calendar.Options.EventClick.bind(null,
												z));
								if (calendar.Options.EventTargetWholeDay) {
									day.className += " clickable";
