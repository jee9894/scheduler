...
<body>
	<nav class="navbar navbar-expand-lg navbar-dark bg-dark"> <a
		class="navbar-brand" href="./">취준생을 위한 스케줄러</a>
	...
	<table border="1">
		...
		<c:forEach var="lec" items="${lec_list}">
			<form action="<c:url value='/assign/insert'/>" method="post">
				<tr>
					<td><a href="<c:url value='/lecture/${lec.lec_id}'/>">
							${lec.lec_id}</a></td>
					<td>${lec.lec_name}</td>
					<td>${lec.lec_start}</td>
					<td>${lec.lec_end}</td>
					<td>
						...
						<input type="text" name="asign_name" placeholder="과제명">
						<input type="date" name="asign_end" value="2999-12-31">
						<input type="submit" value="과제 추가"> 
					</td>
				</tr>
			</form>
		</c:forEach>
		...
		<c:forEach var="assign" items="${assign_list}">
			<form action="<c:url value='/assign/delete'/>" method="post">
				<tr>
					...
					<td><input type="hidden" name="lec_id" value="${assign.lec_id}">
						<input type="hideen" name="asign_name" value="${assign.asign_name}">
